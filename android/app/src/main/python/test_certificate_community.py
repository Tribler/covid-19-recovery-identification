...
class TestCertCommunity(TestBase):
    ...
    def create_node(self):
        return MockIPv8(u"curve25519", CertCommunity,
                        working_directory=u"resource")

    def test_read_certificates_file(self):
        self.assertEqual(os.path.exists('resource/certificates.txt'), False,
                         "Should not be a file here yet")
        self.initialize(CertCommunity, 2)
        self.create_node()
        self.assertEqual(os.path.exists('resource/certificates.txt'), True,
    ...
    def test_read_certificates_file_already_there(self):
        ...
        f.write(json.dumps({"peer1": "cv19"}))
        f.close()
        self.initialize(CertCommunity, 2)
        node = self.create_node()
        self.assertEqual(len(node.get_overlay(CertCommunity).certificates), 1,
                         "empty certificates")
        self.assertEqual(os.path.exists('resource/certificates.txt'), True,
                         "File should still be there")
