import type {State} from '../Store'; // eslint-disable-line no-unused-vars
...
const postLogin = (
  ...
  return fetch(url, data)
      .then((response) => {
        console.log(response);
        if (!response.ok) {
          if (response.status == 417) throw Alert.alert('Not registered');
          if (response.status == 403) throw Alert.alert('wrong credentials');
          else throw Alert.alert('Error while logging in');
        }
        return response.json();
      })
      .then((json) => {
...
const registerLogin = (state: State, password: string, isAttester: boolean) => {
  ...
  return fetch(url, data)
      .then((response) => {
        console.log(response);
        if (!response.ok) {
          if (response.status == 400) throw Alert.alert('Already registered');
          else throw Alert.alert('Error while registering');
        }
        return response.json();
      })
      .then((json) => {
